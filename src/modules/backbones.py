...
class ConvBlock(nn.Module):
    ...
    def __init__(self, indim, outdim, use_elu=False, pool=True, padding=1):
...
class ConvNet(nn.Module):
    def __init__(self, depth, flatten=True, use_elu=False, image_size=84):
        super(ConvNet, self).__init__()
        trunk = []
        for i in range(depth):
            indim = 3 if i == 0 else 64
            outdim = 64
            B = ConvBlock(indim, outdim, use_elu, pool=(i < 4))  # only pooling for fist 4 layers
            trunk.append(B)
...
def Conv4(flatten=True, **args):
    image_size = args.get("image_size", 84)
    use_elu = args.get("use_elu", False)
    return ConvNet(4, flatten=flatten, image_size=image_size, use_elu=use_elu)

def Conv6(flatten=True, **args):
    image_size = args.get("image_size", 84)
    return ConvNet(6, flatten=flatten, image_size=image_size)


def Conv4S(flatten=True, **args):
    image_size = args.get("image_size", 84)
    return ConvNetS(4, flatten=flatten)


def ResNet10(flatten=True, **args):
    return ResNet(SimpleBlock, [1, 1, 1, 1], [64, 128, 256, 512], flatten)


def ResNet18(flatten=True, **args):
    return ResNet(SimpleBlock, [2, 2, 2, 2], [64, 128, 256, 512], flatten)


def ResNet34(flatten=True, **args):
