...
def episodic_collate_fn_wrap(n_way, n_source, n_target):
    ns = n_source
    nt = n_target
    n_way = n_way
    def episodic_collate_fn(input_data):
        ...
        if source == target:
            images_source = torch.cat([input_data[i][0].unsqueeze(0) for i in range(ns*n_way)])
            labels_source = torch.tensor(
                [true_class_ids.index(input_data[i][1]) for i in range(ns*n_way)]
            )
            
            images_target = torch.cat([input_data[i][0].unsqueeze(0) for i in range(ns*n_way, (ns+nt)*n_way)])
            labels_target = torch.tensor(
                [true_class_ids.index(input_data[i][1]) for i in range(ns*n_way, (ns+nt)*n_way)]
            )
